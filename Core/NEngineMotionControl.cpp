								  ...
								  double &control_gain)
{
 ...
 LastAdaptiveTime=env3->GetTime().GetDoubleTime();
...
void NEngineMotionControl::AdditionalComponentsSetup(std::shared_ptr<UNet> net)
{
 if(CheckName("IIPosAfferentGenerator"))
 {
  ...
  gen->Amplitude=1;
  gen->Frequency=0;
  gen->SetCoord(MVector<double,3>(5.0, 15.0, 2.0));
 }
 ...
 if(CheckName("IINegAfferentGenerator"))
 {
  ...
  gen->Amplitude=1;
  gen->Frequency=0;
  gen->SetCoord(MVector<double,3>(5.0, 17.0, 3.0));
 }
 ...
 if(CheckName("AfferentSource1"))
 {
  ...
  gen->Amplitude=0;
  gen->SetCoord(MVector<double,3>(15.0, 2.0, 5.0));
 }
...
void NEngineMotionControl::NewStandardLinksSetup(const string &engine_integrator_name)
{
 ...
 for(size_t k=0;k<Motions.size();k++)
  res&=CreateLink(Motions[k]->GetName()+".MotoneuronL.LTZone","Output",
                 engine_integrator_name,"Inputs");//����� � Pac
