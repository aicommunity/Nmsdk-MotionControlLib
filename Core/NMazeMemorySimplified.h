...
#ifndef NMAZEMEMORYSIMPLIFIED_H
#define NMAZEMEMORYSIMPLIFIED_H
...
class RDK_LIB_TYPE NMazeMemorySimplified: public UNet
{
public: // Параметры
    /// Флаг ситуации с множественным выбором (перекресток, развилка)
    ULProperty<bool, NMazeMemorySimplified, ptPubParameter> Situation;
    ...
    ULProperty<std::vector<int>,NMazeMemorySimplified, ptPubParameter> InputActions;

    ///Тип нейронов действия
    RDK::ULProperty<string, NMazeMemorySimplified, ptPubParameter> ActionNeuronsType;

    /// Количество признаков, описывающих запоминаемую ситуацию
    ULProperty<int, NMazeMemorySimplified, ptPubParameter> FeaturesNum;

    /// Флаг завершения работы (все возможные варианты исследованы)
    ULProperty<bool, NMazeMemorySimplified, ptPubParameter> IsDone;

    ///Вектор параметров ситуации (сейчас = {x,y,alpha}, где alpha - ориентация робота
    ULProperty<MDMatrix<double>, NMazeMemorySimplified, ptPubParameter> SituationCoords;
