...
public: // Параметры
    ...
    UProperty<bool, NMazeMemorySimplified, ptPubParameter> Situation;
    ...
    UProperty<std::vector<int>,NMazeMemorySimplified, ptPubParameter> InputActions;

    ///Тип нейронов действия
    RDK::UProperty<string, NMazeMemorySimplified, ptPubParameter> ActionNeuronsType;

    /// Количество признаков, описывающих запоминаемую ситуацию
    UProperty<int, NMazeMemorySimplified, ptPubParameter> FeaturesNum;

    /// Флаг завершения работы (все возможные варианты исследованы)
    UProperty<bool, NMazeMemorySimplified, ptPubParameter> IsDone;

    ///Вектор параметров ситуации (сейчас = {x,y,alpha}, где alpha - ориентация робота
    UProperty<MDMatrix<double>, NMazeMemorySimplified, ptPubParameter> SituationCoords;
