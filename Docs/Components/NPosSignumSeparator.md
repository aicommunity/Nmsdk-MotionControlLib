...
classDiagram
    ...
    NSignumSeparator <|-- NPosSignumSeparator : Sign=1.0
    class NPosSignumSeparator {
...
- `NPosSignumSeparator` — вариант конфигурации для положительной части

**Примечание:** NPosSignumSeparator создаётся из `NSignumSeparator` с предустановленным `Sign=1.0` при регистрации в библиотеке.
...
sequenceDiagram
    ...
    loop Каждый шаг вычислений
        ...
        Separator->>Source: Output = positive_part
    end
```
...
5. **Вычисление (ACalculate)** - выделение положительной части: если вход отрицательный, выход = 0, иначе выход = Input * Sign * Gain
...
- **Filtering** - фильтрация сигнала (обнуление отрицательных значений)
...
5. Для `NPosSignumSeparator` с `Sign=1.0`: отрицательные значения обнуляются, положительные пропускаются с усилением
...
- **Выходы**: `Output` - положительная часть входного сигнала
...
| `Output` | `MDMatrix<double>` | `ptOutput \| ptPubState` | Выходной сигнал (положительная часть) |
...
#### `ACalculate() -> bool`
**Назначение**: Выделение положительной части входного сигнала.
...
**Для NPosSignumSeparator** (Sign=1.0):
- Отрицательные значения → Output = 0
- Положительные значения → Output = Input * Gain
...
UEPtr<NSignumSeparator> separator = storage->CreateComponent<NSignumSeparator>("PosSep1");
...
```xml
<Component>
    <ClassName>NPosSignumSeparator</ClassName>
    <Name>PosSep1</Name>
    <Properties>
        <Sign>1.0</Sign>
        <Gain>2.0</Gain>
...
Компонент `NPosSignumSeparator` используется в проектах, требующих:
- **Разделения сигналов по знаку** - когда необходимо обрабатывать только положительные значения
- **Управления направлением движения** - для выделения команд движения в одном направлении
...
- Фильтрация сигналов для выделения только положительных компонент

**Связь с другими компонентами:**
- Часто используется вместе с `NNegSignumSeparator` для полного разделения сигнала
...
while (simulation_running) {
    ...
    separator->Calculate();
    double positive_part = separator->Output(0, 0);
}
```
